...
case class PackageInfo(name: String, version: Version, dep: Map[String, String]) {
...
object PackageInfo {
  ...
  def apply(name: String, version: String, dep: Option[Map[String, String]]) =
    new PackageInfo(name, new Version(version), dep.getOrElse(Map.empty[String, String]))

  def apply(name: String, version: String, dep: Map[String, String]) =
    new PackageInfo(name, new Version(version), dep)

  implicit val encoderPackageInfo: Encoder[PackageInfo] =
    Encoder.forProduct3("name", "version", "dep")(p => (p.name, p.version.toString(), p.dep))
